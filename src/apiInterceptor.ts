...
export const apiFetch = async (input: RequestInfo | URL, init?: RequestInit) => {
  ...
  try {
    ...
    if (segments[0] === 'backup-links-to-sheets' && method === 'POST') {
      try {
        const body = JSON.parse(init?.body as string);
        const { sheetId, sheetName, links } = body;
        ...
        const response = await originalFetch(gasUrl, {
          ...
          body: JSON.stringify({
            ...
            sheetId: sheetId,
            sheetName: sheetName || 'LINK',
            links: links || []
          })
        });
        
        const resultText = await response.text();
        console.log(`Backup Links GAS Response:`, resultText);
        
        if (!response.ok) {
           throw new Error(resultText || "Proxy response not ok");
        }
        
        return new Response(JSON.stringify({ success: true, message: 'Links Backup sent to Google Sheets' }), { status: 200 });
      } catch (e: any) {
        console.error("Backup links failed", e);
        return new Response(JSON.stringify({ error: e.message }), { status: 500 });
      }
    }
